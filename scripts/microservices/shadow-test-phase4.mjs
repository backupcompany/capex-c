#!/usr/bin/env node
/**
 * Phase 4 shadow test — configuration leaf service.
 * Usage: make shadow-phase4
 */
const CORE = process.env.NEXT_PUBLIC_CAPEXBE_URL || 'http://127.0.0.1:3001';
const LEAF = process.env.CAPEX_SERVICE_CONFIGURATION_URL || 'http://127.0.0.1:3005';
const PATH = '/configuration/pack';
const ENV_VAR = 'CAPEX_SERVICE_CONFIGURATION_URL';
...
console.log('=== CAPEX Phase 4 shadow test: configuration ===\n');
...
const leafOk = await checkHealth('capex-config (leaf)', LEAF);

if (leafOk) await checkAuthGate('capex-config', LEAF);
...
if (coreOk && leafOk && failures.length === 0) {
  console.log('\nOK  Cutover active — configuration only on leaf service.');
  console.log('    Rollback: re-add ConfigurationModule to capexbe + unset env var');
}

if (failures.length) {
  console.error('\nPhase 4 shadow test FAILED:');
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}

console.log('\nPhase 4 shadow test PASSED');
