#!/usr/bin/env node
/**
 * Phase 6 shadow test — FS cluster leaf service.
 * Usage: make shadow-fs
 */
const CORE = process.env.NEXT_PUBLIC_CAPEXBE_URL || 'http://127.0.0.1:3001';
const LEAF = process.env.CAPEX_SERVICE_FS_URL || 'http://127.0.0.1:3011';
const ENV_VAR = 'CAPEX_SERVICE_FS_URL';
...
console.log('=== CAPEX Phase 6 shadow test: FS cluster ===\n');
...
const leafOk = await checkHealth('capex-fs (leaf)', LEAF);

if (leafOk) {
  for (const path of PATHS) {
    await checkAuthGate('capex-fs', LEAF, path);
  }
}

if (coreOk) {
  for (const path of PATHS) {
    await checkCoreCutover(path);
  }
}

if (coreOk && leafOk && failures.length === 0) {
  console.log('\nOK  Cutover active — FS cluster only on leaf service.');
  console.log('    Executive-summary leaf (:3014) imports FsCoreModule internally.');
  console.log('    Rollback: re-add FsModule + submodules to capexbe + unset env var');
}

if (failures.length) {
  console.error('\nPhase 6 shadow test FAILED:');
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}

console.log('\nPhase 6 shadow test PASSED');
