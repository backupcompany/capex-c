#!/usr/bin/env node
/** Phase 7b shadow test — reporting leaf (dashboard + budget-multi-year). */
const CORE = process.env.NEXT_PUBLIC_CAPEXBE_URL || 'http://127.0.0.1:3001';
const LEAF = process.env.CAPEX_SERVICE_REPORTING_URL || 'http://127.0.0.1:3013';
const ENV_VAR = 'CAPEX_SERVICE_REPORTING_URL';
const PATHS = ['/dashboard/snapshot', '/budget-multi-year/page-bundle'];
const failures = [];
...
console.log('=== CAPEX Phase 7b shadow test: reporting ===\n');
...
if (failures.length) {
  console.error('\nPhase 7b FAILED:', failures);
  process.exit(1);
}
console.log('\nPhase 7b shadow test PASSED');
