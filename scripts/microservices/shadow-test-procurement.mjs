#!/usr/bin/env node
/**
 * Phase 5 shadow test — procurement leaf (PO + GR).
 * Usage: make shadow-procurement
 */
const CORE = process.env.NEXT_PUBLIC_CAPEXBE_URL || 'http://127.0.0.1:3001';
const LEAF = process.env.CAPEX_SERVICE_PROCUREMENT_URL || 'http://127.0.0.1:3010';
const PO_PATH = '/po-update/page-bundle';
const GR_PATH = '/gr-update/page-bundle';
const ENV_VAR = 'CAPEX_SERVICE_PROCUREMENT_URL';

const failures = [];

async function getJson(url, opts = {}) {
  const res = await fetch(url, { ...opts, signal: AbortSignal.timeout(8000) });
  const text = await res.text();
  let json = null;
  try {
    json = text ? JSON.parse(text) : null;
  } catch {
    /* raw */
  }
  return { status: res.status, text, json };
}

async function checkHealth(name, base) {
  try {
    const { status, json } = await getJson(`${base.replace(/\/$/, '')}/health`);
    if (status !== 200) {
      failures.push(`${name} health → HTTP ${status}`);
      return false;
    }
    console.log(`OK  ${name} health → ${JSON.stringify(json)}`);
    return true;
  } catch (e) {
    failures.push(`${name} health unreachable (${base}): ${e instanceof Error ? e.message : e}`);
    return false;
  }
}

async function checkAuthGate(name, base, path) {
  try {
    const { status } = await getJson(`${base.replace(/\/$/, '')}${path}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: 1 }),
    });
    if (status !== 401) {
      failures.push(`${name} ${path} without auth → expected 401, got ${status}`);
      return false;
    }
    console.log(`OK  ${name} ${path} without auth → 401`);
    return true;
  } catch (e) {
    failures.push(`${name} ${path} failed: ${e instanceof Error ? e.message : e}`);
    return false;
  }
}

async function checkCoreCutover(path) {
  try {
    const { status } = await getJson(`${CORE.replace(/\/$/, '')}${path}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: 1 }),
    });
    if (status !== 404) {
      failures.push(`capexbe ${path} after cutover → expected 404, got ${status}`);
      return false;
    }
    console.log(`OK  capexbe ${path} → 404 (cutover — route removed from monolith)`);
    return true;
  } catch (e) {
    failures.push(`capexbe cutover check ${path}: ${e instanceof Error ? e.message : e}`);
    return false;
  }
}

console.log('=== CAPEX Phase 5 shadow test: procurement (PO + GR) ===\n');
console.log(`Core API: ${CORE}`);
console.log(`Leaf API: ${LEAF}\n`);

const override = process.env[ENV_VAR]?.trim();
if (!override) failures.push(`${ENV_VAR} not set (add to capex-apps/.env.local)`);
else console.log(`OK  BFF env ${ENV_VAR}=${override}`);

const coreOk = await checkHealth('capexbe (core)', CORE);
const leafOk = await checkHealth('capex-procurement (leaf)', LEAF);

if (leafOk) {
  await checkAuthGate('capex-procurement', LEAF, PO_PATH);
  await checkAuthGate('capex-procurement', LEAF, GR_PATH);
}

if (coreOk) {
  await checkCoreCutover(PO_PATH);
  await checkCoreCutover(GR_PATH);
}

if (coreOk && leafOk && failures.length === 0) {
  console.log('\nOK  Cutover active — PO + GR only on leaf service.');
  console.log('    Rollback: re-add PoUpdateModule/GrUpdateModule to capexbe + unset env var');
}

if (failures.length) {
  console.error('\nPhase 5 shadow test FAILED:');
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}

console.log('\nPhase 5 shadow test PASSED');
