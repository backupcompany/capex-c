#!/usr/bin/env node
/** Phase 7c shadow test — executive-summary leaf. Usage: make shadow-executive-summary */
const CORE = process.env.NEXT_PUBLIC_CAPEXBE_URL || 'http://127.0.0.1:3001';
const LEAF = process.env.CAPEX_SERVICE_EXECUTIVE_SUMMARY_URL || 'http://127.0.0.1:3014';
const PATHS = ['/executive-summary/page-bundle', '/executive-summary/summary-stats'];
const ENV_VAR = 'CAPEX_SERVICE_EXECUTIVE_SUMMARY_URL';
const failures = [];
...
console.log('=== CAPEX Phase 7c shadow test: executive-summary ===\n');
...
for (const [name, base] of [
  ['capexbe (core)', CORE],
  ['capex-executive-summary (leaf)', LEAF],
]) {
  try {
...
if (failures.length) {
  console.error('\nPhase 7c shadow test FAILED:', failures);
  process.exit(1);
}
console.log('\nPhase 7c shadow test PASSED');
