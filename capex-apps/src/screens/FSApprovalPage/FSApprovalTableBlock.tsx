...
export type FSApprovalTableBlockProps = {
...
function FSApprovalTableSkeleton() {
  return (
...
function FSApprovalTableBlockInner({
  columns,
  data,
  isBlockingLoad,
  showTableBusy,
        ...
        {isBlockingLoad ? (
          <div className="space-y-3">
            <div className="flex flex-col items-center justify-center py-4 text-sm text-siloam-text-secondary gap-2">
              ...
              <span>Memuat data FS Approval…</span>
            </div>
            <FSApprovalTableSkeleton />
          </div>
          ...
          <div className="py-12 text-center text-sm text-siloam-text-secondary space-y-2">
            {isTableError ? (
              ...
              <span>
                Tidak ada FS Proposal yang cocok dengan pencarian
                {debouncedSearch ? ` "${debouncedSearch}"` : ''} atau filter saat ini.
              </span>
            ) : (
              <>
                <p className="font-medium text-siloam-text-primary">
                  Belum ada FS Proposal untuk periode ini
                </p>
                <p className="text-sm max-w-lg mx-auto">
                  Halaman FS Approval menampilkan data dari FS Proposal yang dibuat di{' '}
                  <strong>Budget HU</strong> (bukan checkbox FS Approval di FS Update). Buat atau edit
                  FS Proposal pada project di Budget HU, lalu kembali ke halaman ini.
                </p>
              </>
            )}
          </div>
      ...
      {footerTotalCount > 0 || showTableBusy ? (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 mt-4 border-t border-siloam-border flex-shrink-0">
          <div className="text-sm text-siloam-text-secondary">
            ...
            {Math.min(currentPage * itemsPerPage, footerTotalCount)} of {footerTotalCount} proposals
          </div>
...
export const FSApprovalTableBlock = memo(FSApprovalTableBlockInner);
FSApprovalTableBlock.displayName = 'FSApprovalTableBlock';
