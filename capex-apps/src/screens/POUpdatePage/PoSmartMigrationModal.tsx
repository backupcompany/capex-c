...
const MIGRATION_TARGET = 'PoUpdates' as const;
...
export type PoSmartMigrationModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  currentUser: User;
...
function PoSmartMigrationModalInner({
  isOpen,
  onClose,
  onSuccess,
  currentUser,
  showToast,
}: PoSmartMigrationModalProps) {
  ...
  const handleExecute = useCallback(async () => {
    ...
    if (!isMappingValid()) {
      showToast('Mapping belum lengkap. Pastikan kolom Asset Code (*) sudah dipetakan.', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.successCount > 0) {
        showToast(
          res.successCount === 1
            ? '1 aset PO berhasil diperbarui dari Excel.'
            : `${res.successCount} aset PO berhasil diperbarui dari Excel.`,
          'success',
        );
        onSuccess();
      }
  ...
  }, [file, isMappingValid, mapping, currentUser, showToast, onSuccess]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="po-smart-migration-title"
    >
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col rounded-xl border border-siloam-border bg-siloam-surface shadow-soft">
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-siloam-border p-5">
          <div>
            <h3 id="po-smart-migration-title" className="text-lg font-bold text-siloam-text-primary flex items-center gap-2">
              <FileSpreadsheet className="h-5 w-5 text-siloam-blue" />
              Smart Migration — PO Data
            </h3>
            <p className="mt-1 text-sm text-siloam-text-secondary">
              Import CPR ID, PO Number, Tgl PO, dan PO Value dari Excel. Asset dicocokkan berdasarkan Kode Asset.
            </p>
          </div>
          <button
            type="button"
              ...
              <div className="flex items-center justify-between rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-4">
                <div>
                  ...
                  <p className="text-xs text-blue-700">
                    Download template Excel dengan kolom Asset Code, CPR ID, PO Number, Tgl PO, PO Value.
                  </p>
                </div>
                <button
                  type="button"
          ...
          {currentStep === 2 ? (
            <div className="space-y-4">
              <p className="text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-lg p-3">
                Kolom dipetakan otomatis. Pastikan <strong>Asset Code *</strong> sudah benar, lalu sesuaikan
                CPR ID / PO Number / PO Value jika perlu.
              </p>
              ...
              {!isMappingValid() ? (
                <p className="text-sm text-danger">Kolom wajib Asset Code (*) belum dipetakan.</p>
              ) : null}
            </div>
          ) : null}
                ...
                <div className="rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-siloam-text-primary">
                        {migrationProgress.message || 'Memproses migrasi PO…'}
                      </p>
                  ...
                  {migrationProgress.stage === 'saving' ? (
                    <p className="text-xs text-blue-800">
                      Data PO disimpan bertahap ke database — baris yang sudah masuk tetap tersimpan meski
...
export const PoSmartMigrationModal = memo(PoSmartMigrationModalInner);
