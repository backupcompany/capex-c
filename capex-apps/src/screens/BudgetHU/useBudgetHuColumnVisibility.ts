...
function loadStoredVisibleIds(): Set<BudgetHuTableColumnId> {
  ...
  for (const storage of [window.sessionStorage, window.localStorage]) {
    try {
      const raw = storage.getItem(BUDGET_HU_COLUMN_STORAGE_KEY);
      if (!raw) continue;
      const parsed = JSON.parse(raw) as { visibleIds?: string[] };
      if (!Array.isArray(parsed.visibleIds) || parsed.visibleIds.length === 0) continue;
      ...
      if (withPinned.size > BUDGET_HU_PINNED_COLUMN_IDS.length) return withPinned;
    } catch {
      /* ignore corrupt / quota read */
    }
  }
  return new Set(BUDGET_HU_DEFAULT_VISIBLE_COLUMN_IDS);
}
...
function persistVisibleIds(ids: Set<BudgetHuTableColumnId>) {
  ...
  const payload = JSON.stringify({ visibleIds: toggleable });
  for (const storage of [window.sessionStorage, window.localStorage]) {
    try {
      storage.setItem(BUDGET_HU_COLUMN_STORAGE_KEY, payload);
      return;
    } catch {
      /* quota — try other storage or skip */
    }
  }
}
