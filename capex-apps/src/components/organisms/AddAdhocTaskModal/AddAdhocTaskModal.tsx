...
import type { User, EnrichedAsset, Project } from '../../../types';
import * as taskService from '../../../services/taskService';
import { useToast } from '../../../contexts/ToastContext';
import { Spinner } from '../../atoms/Spinner/Spinner';
import {
  filterAssignableUsers,
  userAssignmentRoleLabel,
} from './assignableUsers';
...
export const AddAdhocTaskModal: React.FC<AddAdhocTaskModalProps> = ({
  isOpen,
  onClose,
  assetId,
  asset,
  ...
  const huHint = asset?.huName || undefined;

  const filteredUsers = useMemo(
    () => filterAssignableUsers(allUsers, { query: userQuery, huHint }),
    [allUsers, userQuery, huHint],
  );

  const selectedUser = useMemo(
    () => filteredUsers.find((u) => u.id === assignedToUserId) ??
      allUsers.find((u) => u.id === assignedToUserId),
    [filteredUsers, allUsers, assignedToUserId],
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]">
      <div className="bg-siloam-surface p-6 rounded-xl shadow-soft w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div ref={pickerRef} className="relative">
            <label htmlFor="assignee-search" className="block text-sm font-medium text-siloam-text-secondary">
            ...
            <input
              ...
              type="search"
              autoComplete="off"
              value={pickerOpen ? userQuery : selectedUser?.username ?? userQuery}
              onChange={(e) => {
                setUserQuery(e.target.value);
                setPickerOpen(true);
                if (assignedToUserId != null) setAssignedToUserId(null);
              }}
              onFocus={() => {
                setPickerOpen(true);
              ...
              className="mt-1 block w-full border border-siloam-border rounded-xl p-2 bg-siloam-surface focus:outline-none focus:ring-2 focus:ring-siloam-blue"
            />
            {selectedUser && !pickerOpen ? (
              <p className="mt-1 text-xs text-siloam-text-secondary">
                Role: {userAssignmentRoleLabel(selectedUser) || '—'}
              </p>
            ) : null}
            {pickerOpen ? (
              <ul
                className="absolute z-[110] mt-1 w-full max-h-56 overflow-y-auto rounded-xl border border-siloam-border bg-siloam-surface shadow-xl py-1"
                role="listbox"
              >
                {filteredUsers.length === 0 ? (
                  <li className="px-3 py-2 text-sm text-siloam-text-secondary">
                    {allUsers.length === 0
                      ? 'Daftar user belum dimuat. Tutup modal lalu buka lagi.'
                      : 'Tidak ada user cocok (perlu id numerik + role di DB).'}
                  </li>
                ) : (
                  filteredUsers.map((user) => {
                    const role = userAssignmentRoleLabel(user);
                    return (
                      <li key={user.id}>
                        <button
                          type="button"
                          role="option"
                          aria-selected={user.id === assignedToUserId}
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-siloam-bg ${
                            user.id === assignedToUserId ? 'bg-siloam-blue/10 text-siloam-blue' : ''
                          }`}
                          onClick={() => {
                        ...
                        >
                          <span className="font-medium text-siloam-text-primary">{user.username}</span>
                          {role ? (
                            <span className="block text-xs text-siloam-text-secondary">{role}</span>
                          ) : null}
                        </button>
                      </li>
                    );
          ...
          <div>
            <label htmlFor="due-date" className="block text-sm font-medium text-siloam-text-secondary">
              Due Date
            </label>
            <input
              type="date"
