...
for (const file of await walk(DIST_SRC)) {
  ...
  next = next.replaceAll('../../../packages/capex-audit-core/', '../packages/capex-audit-core/');
...
await link(join(ROOT, '../../packages/capex-audit-core'), join(DIST, 'packages/capex-audit-core'));

console.log('OK  postbuild: dist symlinks (capexbe + auth-core + audit-core) + paths');
